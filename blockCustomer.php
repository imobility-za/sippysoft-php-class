...
try {
    ...
    // Example: Block customer
    try {
        $response = $client->blockCustomer(190); // Replace with actual account ID
        echo "Customer info: " . json_encode($response, JSON_PRETTY_PRINT) . "\n";
    } catch (Exception $e) {
        echo "Error customer account : " . $e->getMessage() . "\n";
    }

    // Example: Unblock account
    try {
        $response = $client->unblockCustomer(190); // Replace with actual account ID
        echo "Customer info: " . json_encode($response, JSON_PRETTY_PRINT) . "\n";
    } catch (Exception $e) {
        echo "Error customer account : " . $e->getMessage() . "\n";
    }
